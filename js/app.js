...
function resizeViewer() {
  ...
  if (typeof redrawZones === "function") redrawZones();
  stage.update();
}
...
function _drawRobot(ctx, px, py, yaw, scaleX, forceVisible) {
  ...
  /* Body — Sleek light violet rounded capsule robot chassis */
  var bodyColor = RobotConfig.robot.color || "#a78bfa";
  ctx.fillStyle = bodyColor;
  ctx.strokeStyle = "#ffffff";
  ctx.lineWidth = 2.5;
  ctx.beginPath();
  var r = Math.min(L, W) * 0.25;
  ctx.moveTo(-hl + r, -hw);
  ctx.lineTo(hl - r, -hw);
  ...
  /* Side tracks/lights for futuristic SCADA aesthetics */
  ctx.fillStyle = "#3b82f6"; /* high-tech battery indicator glow */
  ctx.fillRect(-hl * 0.4, -hw - 2.5, hl * 0.8, 2.5);
  ctx.fillRect(-hl * 0.4, hw, hl * 0.8, 2.5);

  /* Forward direction indicator — white triangle pointing forward (+X) */
  ctx.fillStyle = "rgba(255,255,255,0.95)";
  ctx.beginPath();
  ctx.moveTo(hl * 0.65, 0);
  ctx.lineTo(-hl * 0.15, -hw * 0.4);
  ctx.lineTo(-hl * 0.15, hw * 0.4);
  ctx.closePath();
  ctx.fill();
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(hl - r * 0.5, -hw * 0.6);
  ctx.lineTo(hl - r * 0.5, hw * 0.6);
  ctx.stroke();
...
(function buildGoalArrow() {
  goalArrowContainer.alpha = 0.5; /* Beautifully faded preview symbol */
  ...
  goalArrowContainer.addChild(body);
  goalArrowContainer.addChild(leftTrack);
  goalArrowContainer.addChild(rightTrack);
  goalArrowContainer.addChild(dir);
  goalArrowContainer.addChild(front);
})();
...
var SCAN_COLOR = "rgba(239, 68, 68, 0.85)";  /* blurry red */
...
function drawScan(scan, robotX, robotY, robotYaw) {
  ...
  _scanCtx.shadowColor = "rgba(239, 68, 68, 1)";
  _scanCtx.shadowBlur = 8;
...
/* ==========================================================================
   GLOBAL PROCESS FEEDBACK & DYNAMIC HMI INDICATOR LAYER
   ========================================================================== */
(function () {
  const PROCESS_MAPPING = {
    '/start_robot': 'Starting AMR Onboard Drivers & Bringup',
    '/stop_robot': 'Shutting Down AMR Drivers & LiDARS',
    '/start_mapping': 'Initializing SLAM Mapping Service',
    '/stop_mapping': 'Stopping SLAM Mapping Service',
    '/save_map': 'Saving 2D Occupancy Grid Map',
    '/map/delete': 'Deleting Saved Map File',
    '/start_localization': 'Loading Navigation Stack & AMCL Localization',
    '/navigate_to_pose': 'Dispatching Autonomous Navigation Goal',
    '/cancel_goal': 'Aborting Autonomous Navigation Command',
    '/dock': 'Initiating Precision Docking Sequence',
    '/undock': 'Initiating Undocking & Charging Release',
    '/mission/save': 'Saving Waypoint Mission Configuration',
    '/mission/start': 'Launching Autonomous Waypoint Mission',
    '/mission/stop': 'Halt Autonomous Waypoint Mission',
    '/mission/resume': 'Resuming Paused Waypoint Mission',
    '/mission/pause': 'Safe Halting Waypoint Mission',
    '/run_program': 'Executing Python Automation Script',
    '/stop_program': 'Terminating Python Script Execution',
    '/zones/save': 'Saving Restricted No-Go Zones',
    '/zones/reload_costmap': 'Syncing Navigation Costmap Zones',
    '/delivery/start': 'Launching Delivery Automation Cycle',
    '/delivery/stop': 'Terminating Delivery Service'
  };
  ...
  window.fetch = function (input, init) {
    ...
    let matchedKey = null;
    for (let key in PROCESS_MAPPING) {
    ...
    if (matchedKey) {
      const processName = PROCESS_MAPPING[matchedKey];
      activeProcessesCount++;

      if (typeof window.showLoadingOverlay === 'function') {
        window.showLoadingOverlay("Executing: " + processName + "...");
      }

      if (typeof window.showToast === 'function') {
        window.showToast("⚙️ Initiating " + processName.toLowerCase() + "...", "info");
      }

      return new Promise(function (resolve, reject) {
        const timeoutId = setTimeout(function () {
          if (typeof window.showToast === 'function') {
            window.showToast("⏳ " + processName + " is taking longer than expected. Please wait...", "info");
          }
          const messageEl = document.getElementById("global-loading-message");
          if (messageEl) {
            messageEl.innerHTML = processName.toUpperCase() + "<br><span style='font-size: 10px; color: var(--yellow); letter-spacing: 1px;'>TAKING TIME — CONTINUOUS SYSTEM PROCESSING...</span>";
          }
        }, 4000);

        originalFetch.apply(window, [input, init])
          .then(function (response) {
            clearTimeout(timeoutId);

            if (!response.ok) {
              activeProcessesCount = Math.max(0, activeProcessesCount - 1);
              if (activeProcessesCount === 0 && typeof window.hideLoadingOverlay === 'function') {
                window.hideLoadingOverlay();
              }
              if (typeof window.showToast === 'function') {
                window.showToast("❌ " + processName + " failed: Status " + response.status, "error");
              }
              resolve(response);
              return;
            }
            ...
            responseClone.json()
              .then(function (data) {
                if (data && (data.status === "error" || data.error || data.success === false)) {
                  ...
                  if (typeof window.showToast === 'function') {
                    window.showToast("❌ " + processName + " failed: " + (data.message || 'Error occurred'), "error");
                  }
                  resolve(response);
                  return;
                }

                executeReadinessSequence(matchedKey, processName, response, resolve);
              })
              .catch(function () {
                executeReadinessSequence(matchedKey, processName, response, resolve);
              });
          ...
          .catch(function (error) {
            ...
            if (typeof window.showToast === 'function') {
              window.showToast("❌ " + processName + " failed: Connection lost.", "error");
            }
            reject(error);
          });
      });
  ...
  function executeReadinessSequence(key, processName, originalResponse, resolvePromise) {
    const messageEl = document.getElementById("global-loading-message");

    function finish(isSuccess = true) {
      activeProcessesCount = Math.max(0, activeProcessesCount - 1);
      if (activeProcessesCount === 0 && typeof window.hideLoadingOverlay === 'function') {
        window.hideLoadingOverlay();
      }
      if (isSuccess && typeof window.showToast === 'function') {
        window.showToast("✅ " + processName + " completed successfully.", "success");
      }
      resolvePromise(originalResponse);
    }

    // A. 5-second countdown for robot start
    if (key === '/start_robot') {
      let secondsLeft = 5;
      const interval = setInterval(function () {
        secondsLeft--;
        if (messageEl) {
          messageEl.innerHTML = `STARTING AMR ONBOARD DRIVERS...<br><span style='font-size:12px;color:var(--accent);letter-spacing:1px;'>HARDWARE BRINGUP IN PROGRESS [${secondsLeft}s LEFT]</span>`;
        }
        if (secondsLeft <= 0) {
      ...
      if (messageEl) {
        messageEl.innerHTML = `STARTING AMR ONBOARD DRIVERS...<br><span style='font-size:12px;color:var(--accent);letter-spacing:1px;'>HARDWARE BRINGUP IN PROGRESS [5s LEFT]</span>`;
      }
      return;
    }

    // B. 3-second teardown delay for stop_robot, stop_localization, stop_mapping
    if (key === '/stop_robot' || key === '/stop_localization' || key === '/stop_mapping') {
      let secondsLeft = 3;
      const interval = setInterval(function () {
        secondsLeft--;
        if (messageEl) {
          messageEl.innerHTML = `CLEANING UP SYSTEM STACKS...<br><span style='font-size:12px;color:var(--red);letter-spacing:1px;'>TERMINATING ACTORS & PERIPHERALS [${secondsLeft}s LEFT]</span>`;
        }
        if (secondsLeft <= 0) {
          clearInterval(interval);
          finish();
      ...
      if (messageEl) {
        messageEl.innerHTML = `CLEANING UP SYSTEM STACKS...<br><span style='font-size:12px;color:var(--red);letter-spacing:1px;'>TERMINATING ACTORS & PERIPHERALS [3s LEFT]</span>`;
      }
      return;
    }

    // C. Nav2 and AMCL Warmup Polling for start_localization
    if (key === '/start_localization') {
      let tries = 0;
      const maxTries = 20; // 30 seconds max
      if (messageEl) {
        messageEl.innerHTML = `LAUNCHING NAVIGATION & AMCL...<br><span style='font-size:11px;color:var(--yellow);letter-spacing:1px;'>WARMING UP STACK (0%)</span>`;
      }

      function pollAmclReady() {
        originalFetch(SERVER_URL + "/amcl/ready")
          .then(r => r.json())
          .then(function (d) {
            tries++;
            const pct = Math.min(99, Math.floor((tries / maxTries) * 100));
            if (messageEl) {
              messageEl.innerHTML = `LAUNCHING NAVIGATION & AMCL...<br><span style='font-size:11px;color:var(--yellow);letter-spacing:1px;'>WARMING UP STACK (${pct}%)</span>`;
            }

            if (d.ready) {
              if (messageEl) {
                messageEl.innerHTML = `LAUNCHING NAVIGATION & AMCL...<br><span style='font-size:11px;color:var(--green);letter-spacing:1px;'>READY (100%)</span>`;
              }
              setTimeout(function () { finish(); }, 500);
            } else if (tries >= maxTries) {
              finish();
            } else {
              setTimeout(pollAmclReady, 1500);
            }
          })
          .catch(function () {
            tries++;
    ...
    // D. SLAM async check for start_mapping
    if (key === '/start_mapping') {
      let tries = 0;
      const maxTries = 10;
      if (messageEl) {
        messageEl.innerHTML = `LAUNCHING SLAM TOOLBOX...<br><span style='font-size:11px;color:var(--yellow);letter-spacing:1px;'>SPAWNING MAP CLIENT (0%)</span>`;
      }

      function pollSlamReady() {
        originalFetch(SERVER_URL + "/map_meta")
          .then(function (r) { return r.status === 204 ? null : r.json(); })
          .then(function (meta) {
            tries++;
            const pct = Math.min(99, Math.floor((tries / maxTries) * 100));
            if (messageEl) {
              messageEl.innerHTML = `LAUNCHING SLAM TOOLBOX...<br><span style='font-size:11px;color:var(--yellow);letter-spacing:1px;'>SPAWNING MAP CLIENT (${pct}%)</span>`;
            }

            if (meta && meta.status === "ok" && meta.seq >= 0) {
              if (messageEl) {
                messageEl.innerHTML = `LAUNCHING SLAM TOOLBOX...<br><span style='font-size:11px;color:var(--green);letter-spacing:1px;'>READY (100%)</span>`;
              }
              setTimeout(function () { finish(); }, 500);
            } else if (tries >= maxTries) {
              finish();
            } else {
              setTimeout(pollSlamReady, 1500);
            }
          })
          .catch(function () {
            tries++;
    ...
    // E. Short buffer delays to let server complete disk writes
    if (key === '/save_map') {
      if (messageEl) {
        messageEl.innerHTML = `SAVING OCCUPANCY GRID MAP...<br><span style='font-size:11px;color:var(--accent);letter-spacing:1px;'>WRITING PGM AND YAML METADATA TO STORAGE</span>`;
      }
    ...
    if (key === '/map/delete') {
      if (messageEl) {
        messageEl.innerHTML = `DELETING MAP FILES...<br><span style='font-size:11px;color:var(--red);letter-spacing:1px;'>CLEANING FILTERS AND SIDE-CAR ZONE SCHEMAS</span>`;
      }
  ...
  // Setup dynamic topbar active task checking loop
  setInterval(function () {
    var indicator = document.getElementById("active-task-indicator");
    var taskName = document.getElementById("active-task-name");
    if (!indicator || !taskName) return;

    var activeTasks = [];
    if (typeof mapMode !== 'undefined' && mapMode === 'mapping') {
      activeTasks.push("SLAM Mapping");
    } else if (typeof mapMode !== 'undefined' && mapMode === 'localization') {
      if (typeof missionRunning !== 'undefined' && missionRunning) {
        if (typeof missionPaused !== 'undefined' && missionPaused) {
          activeTasks.push("Mission Paused");
        } else {
          activeTasks.push("Autonomous Mission");
        }
      } else {
        activeTasks.push("Localization Active");
      }
    }

    // Check if delivery mode is active
    var deliveryToggle = document.getElementById("deliveryModeToggle");
    if (deliveryToggle && deliveryToggle.checked) {
      activeTasks.push("Delivery Cycle");
    }

    // Check if Python program is running
    var programStatus = document.getElementById("program-status");
    if (programStatus && programStatus.textContent.toLowerCase().indexOf("running") !== -1) {
      activeTasks.push("Automation Script");
    }

    if (activeTasks.length > 0) {
      taskName.textContent = activeTasks.join(" + ").toUpperCase();
