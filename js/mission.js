...
function startMission() {
  ...
  var cycleRange = document.getElementById("mission-cycle-range");
  var cycleVal = cycleRange ? parseInt(cycleRange.value) : 1;
  var cycles = (cycleVal === 11) ? -1 : cycleVal;
...
function _uiRunning() {
  ...
  var rangeSlider = document.getElementById("mission-cycle-range");
...
window.initMissionsView = function() {
  ...
  // Connect range cycles slider
  var range = document.getElementById("mission-cycle-range");
  if (range) {
    range.addEventListener("input", function() {
      updateCycleDisplay(this.value);
    });
    updateCycleDisplay(range.value);
  }
};
